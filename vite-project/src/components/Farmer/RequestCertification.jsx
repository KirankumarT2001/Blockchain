...
import CertificationReq from "../../Customhooks/certificate.jsx";

const RequestCertification = () => {
  const {applyCertificate} = CertificationReq();
  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm();
  const onSubmit = async ({cropname,cropquantity,cropquality,desiredprice}) => {
    await applyCertificate(cropname,cropquantity,cropquality,desiredprice);
  };
  return (
    <>
      <img
